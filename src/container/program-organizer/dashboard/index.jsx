...
import { getAllDashboardData } from "../../../services/http-services/organzier";
...
const PODashboard = () => {
  ...
  const [state, setState] = useState({ totalProject: 0, totalSupervisor: 0 });
  ...
  const getDashboardData = () => {
    getAllDashboardData({
      ...
      cbSuccess: ({ _projects }) => {
        ...
        setState({ ...state, totalProject: _projects.length });
      },
      cbFailure: (error) => {
            ...
            <div className='flex-auto p-4'>
              <div className='-mx-3 flex flex-row'>
                <div className='w-2/3 max-w-full flex-none px-3'>
                  <div>
                    <p className='mb-0 font-sans text-sm font-semibold leading-normal'>Total Projects</p>
                    <h5 className='mb-0 font-bold'>{state.totalProject}</h5>
                  </div>
                </div>
                <div className='basis-1/3 px-3 text-right'>
                  <div className='inline-block h-12 w-12 rounded-lg bg-gradient-to-tl from-blue-600 to-cyan-400 text-center'>
                    <i
                      className='fa fa-file-code-o fa-lg relative top-3 leading-none text-white'
                      aria-hidden='true'
                    ></i>
                  </div>
                </div>
          ...
          <div className='shadow-soft-xl relative flex min-w-0 flex-col break-words rounded-2xl bg-white bg-clip-border'>
            <div className='flex-auto p-4'>
              <div className='-mx-3 flex flex-row'>
                <div className='w-2/3 max-w-full flex-none px-3'>
                  <div>
                    <p className='mb-0 font-sans text-sm font-semibold leading-normal'>Total Students</p>
                ...
                <div className='basis-1/3 px-3 text-right'>
                  <div className='inline-block h-12 w-12 rounded-lg bg-gradient-to-tl from-blue-600 to-cyan-400 text-center'>
                    <i
                      className='fa fa-graduation-cap fa-lg relative top-3 leading-none text-white'
                      aria-hidden='true'
                    ></i>
                  </div>
                </div>
          ...
          <div className='shadow-soft-xl relative flex min-w-0 flex-col break-words rounded-2xl bg-white bg-clip-border'>
            <div className='flex-auto p-4'>
              <div className='-mx-3 flex flex-row'>
                <div className='w-2/3 max-w-full flex-none px-3'>
                  <div>
                    <p className='mb-0 font-sans text-sm font-semibold leading-normal'>Total Supervisors</p>
                    <h5 className='mb-0 font-bold'>{state.totalProject}</h5>
                  </div>
                </div>
                <div className='basis-1/3 px-3 text-right'>
                        ...
                        <tr>
                          ...
                          <td className='whitespace-nowrap border-b bg-transparent p-2 align-middle'>
                            <ScrollContainer style={{ cursor: "grab" }}>
                              <div className='flex'>
                                {pro.teamsData?.map((developerName) => (
                                  <div className={getBadgeClasses()}>
                                    <p className='m-0 p-0'>{developerName}</p>
                                  </div>
                                ))}
                              </div>
                            </ScrollContainer>
