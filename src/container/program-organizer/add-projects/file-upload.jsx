...
const FileUpload = ({ imageFile }) => {
  ...
  const [selectedFileDetails, setSelectedFileDetails] = useState({ name: "", type: "", size: "" });
  const [fileFormat, setFileFormat] = useState(null);

  const changeHandler = (event) => {
    let getFiles = event.target.files[0];
    console.log("🚀 ~ file: file-upload.jsx ~ line 12 ~ changeHandler ~ event", event.target.files);
          ...
          <li className='row'>
            <div className='content'>
              ...
              <div className='details'>
                <span className='name'>{selectedFileDetails.name} • Uploaded</span>
